 ...
 * @return boolean
 */
function forminator_old_field( $element_id, $fields ) {
	$element_id  = forminator_remove_prefixes( $element_id );
	$element_ids = wp_list_pluck( $fields, 'element_id' );

	return ! in_array( $element_id, $element_ids, true );
}
...
function forminator_allowed_mime_types( $allow = true ) {
	...
	if ( ! $allow ) {
		$filters = array( 'htm|html', 'js', 'swf', 'exe' );
